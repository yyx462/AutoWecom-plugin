...
## End-of-day batch (sum to 8h)

When the user asks to 总结今天 / 报工总结 / close out the day — or runs
parallel agent sessions all day and reports at once:
...
3. Fit them to one working day — pipe a JSON array to
   ...
   proportions, snaps to 0.5h, and lands Σ on EXACTLY 8.0h; it prints
   the fitted table plus ready `log-labor add` lines (with `--date`).
