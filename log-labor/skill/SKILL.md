...
- **A DIFFERENT sheet (own webhook key + field ids)** — TRIGGER: the user
  pastes a webhook URL/key that is not the configured one (「报到我们组的
...
<!-- log-labor:dev begin (managed by `log-labor skill` — do not edit) -->
<!-- log-labor end -->
